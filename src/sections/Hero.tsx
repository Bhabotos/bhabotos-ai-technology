import { Bot, Database, TrendingUp, User, Workflow } from 'lucide-react'
import Button from '../components/Button'
import WorkflowVisual from '../components/WorkflowVisual'

const FLOW_NODES = [
  { icon: User, label: 'User' },
  { icon: Bot, label: 'AI' },
  { icon: Workflow, label: 'Automation' },
  { icon: Database, label: 'Data' },
  { icon: TrendingUp, label: 'Business Result' },
]

export default function Hero() {
  return (
    <section id="top" className="relative overflow-hidden scroll-mt-16">
      <div className="bg-grid pointer-events-none absolute inset-0 [mask-image:radial-gradient(ellipse_60%_60%_at_50%_0%,black,transparent)]" aria-hidden="true" />

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-28">
        <div className="grid lg:grid-cols-2 gap-14 lg:gap-10 items-center">
          <div className="flex flex-col items-start text-left animate-fade-up">
            <span className="text-xs font-semibold uppercase tracking-[0.2em] text-accent">
              AI • AUTOMATION • NETWORK • DEVOPS
            </span>

            <h1 className="mt-5 text-4xl sm:text-5xl lg:text-6xl font-semibold text-heading leading-[1.08] tracking-tight">
              Learn AI.
              <br />
              Automate Work.
              <br />
              <span className="text-gradient">Build Solutions.</span>
            </h1>

            <p className="mt-6 text-base sm:text-lg text-text max-w-xl leading-relaxed">
              Practical AI, Automation, Network & DevOps training and technology solutions for professionals and
              businesses.
            </p>
            <p className="mt-3 text-sm sm:text-base text-text-muted max-w-xl leading-relaxed">
              Learn practical skills. Build real projects. Automate real work.
            </p>

            <div className="mt-8 flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              <Button to="/training" variant="primary" size="lg">
                Explore Training
              </Button>
              <Button to="/#solutions" variant="secondary" size="lg">
                Get a Solution
              </Button>
            </div>
          </div>

          <WorkflowVisual
            nodes={FLOW_NODES}
            ariaLabel="Workflow: User leads to AI, which leads to Automation, which leads to Data, which leads to Business Result"
          />
        </div>
      </div>
    </section>
  )
}
